# Two Independent sample - unpooled test when population standard deviations are unknow and 
# assumed sigma1 != sigma2 (t-test)

unpooled = function(x1,x2,du,s1,s2,n1,n2,type,alpha){
  ...
  if (type == 'Two Tail') {
    plot(x, hx, type="n", ylab="",xlab = "dMu",
         main="Two Tail Independent Sample Unpooled Variance Test", axes=FALSE)
    if (p_value > 0.5) {
      p_value = 2*(1-p_value)
  ...
  if (type == 'Left Tail') {
    plot(x, hx, type="n", ylab="",xlab = "dMu",
         main="Left Tail Independent Sample Unpooled Variance Test", axes=FALSE)
    t1 = paste("P-value:",p_value)
  ...
  if (type == 'Right Tail') {
    plot(x, hx, type="n", ylab="",xlab = "dMu",
         main="Right Tail Independent Sample Unpooled Variance Test", axes=FALSE)
